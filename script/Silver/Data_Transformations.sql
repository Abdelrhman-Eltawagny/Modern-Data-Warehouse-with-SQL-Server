...
BEGIN
	...
	SET @batch_start_time = GETDATE();
		...
		END cst_gndr,
		cst_create_date
		-- Remove Duplicates and Nulls in cst_id 
		FROM (
		...
		-- Map product line to descriptive values 
		...
		-- Fix sales order date and convert it to Date 
		CASE 
			WHEN sls_order_dt = 0 OR LEN(sls_order_dt) != 8 THEN NULL
			ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE)
		END AS sls_order_dt,

		-- Fix sales ship date and convert it to Date 
		CASE 
			WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8 THEN NULL
			ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
		END AS sls_ship_dt,

		-- Fix sales due date and convert it to Date 
		CASE 
		...
		CASE 
			WHEN sls_sales IS NULL OR sls_price <= 0
		...
		-- Normalize gender values and handle unkown cases
		CASE 
		...
		-- Replace '-' in Primary key
		SELECT REPLACE(cid, '-' , '')  AS cid,
		-- Normalize and Handle missing or blank country
		CASE 
